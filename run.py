...
from sse_pt.model import SSEPT
from sse_pt.data import WarpSampler
from sse_pt.utils import set_seed, data_partition
from sse_pt.train import train
from sse_pt.eval import evaluate

set_seed(42)

parser = argparse.ArgumentParser(description='SSE-PT training')
# data
...
model_cfg.add_argument('--sse_prob_user', type=float, default=0.0,
                       help='The probability to replace the user embedding.')
model_cfg.add_argument('--sse_prob_item', type=float, default=0.0,
                       help='The probability to replace the item embedding.')
# device
...
experiment.add_argument('--model_name', default='SSE_PT', help='used for logging and save models.')
...
experiment.add_argument('--val_start_batch', type=int, default=1000,
                        help='how many steps to wait before vaidation')
...
def main():
    ...
    # model
    model = SSEPT(user_num, item_num, args)
    print(model)

    if not args.test:  # train
        # dataloader
        sampler = WarpSampler(user_train, user_num, item_num, batch_size=args.batch_size, maxlen=args.maxlen,
                              n_workers=args.num_workers, sse_prob_user=args.sse_prob_user, sse_prob_item=args.sse_prob_item)
