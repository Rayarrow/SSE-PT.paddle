...
from sse_pt.utils import logger
...
def train(sampler, model, args, num_batch, dataset):
    ...
    if args.optimizer == 'Adam':
        ...
        raise TypeError("Unknown optimizer {}".format(args.optimizer))
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        for i_batch in range(num_batch):
            tot_batch += 1
            user_ids, seq, pos, neg = sampler.next_batch()  # tuples to ndarray
            user_ids, seq, pos, neg = paddle.to_tensor(user_ids, dtype='int64'), paddle.to_tensor(seq, dtype='int64'), \
                    paddle.to_tensor(pos), paddle.to_tensor(neg)
            pos_logits, neg_logits = model(user_ids, seq, pos, neg)  # ()
            ...
            if tot_batch >= args.val_start_batch and tot_batch % args.val_interval == 0 and i_batch != 0:
                ...
                if best_pair is None or valid_pair > best_pair:
                    ...
                    file_path = '%s/%s_best.pth.tar' % (args.model_name, args.save_folder)
                    logger.info("=> found better validated model, saving to %s" % file_path)
        ...
        logger.info('Epoch {:3} - loss: {:.4f}  lr: {:.5f}'.format(epoch,
                                                             epoch_loss / num_batch,
                                                             optim._learning_rate,
                                                             ))

        if args.checkpoint and epoch % args.save_interval == 0:
            file_path = '%s/%s_epoch_%d.pth.tar' % (args.save_folder, args.model_name, epoch)
            logger.info("\r=> saving checkpoint model to %s" % file_path)
