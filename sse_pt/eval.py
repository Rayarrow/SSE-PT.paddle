...
def evaluate(dataset, model, epoch_train, batch_train, args, is_val=True):
    ...
    for u in tqdm(users):
        ...
        rated = set(before[u])
        rated.add(0)
        item_indexes = [now[u][0]]
        for _ in range(100):
            t = np.random.randint(1, itemnum + 1)
            while t in rated:
                t = np.random.randint(1, itemnum + 1)
            item_indexes.append(t)

        predictions = -model.predict(*[paddle.to_tensor(l) for l in [[u], [seq], item_indexes]]) # `[u]` and `[seq]` adds batch_size[1] at axis[0].
